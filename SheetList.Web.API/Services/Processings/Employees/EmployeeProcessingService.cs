...
using SheetList.Web.API.Models.Exceptions;
...
namespace SheetList.Web.API.Services.Processings.Employees
{
        public class EmployeeProcessingService : IEmployeeProcessingService
        {
            ...
            public FileConfiguration ConvertSqlDataToXmlFile()
            {
                ...
                string downloadsFolder = Path.Combine(this.hostingEnvironment.WebRootPath, "downloads");
                ...
                string pathOfXmlFile = Path.Combine(downloadsFolder, generateXmlFileName);

                ValueTask<IQueryable<Employee>> employees = this.employeeService.RetrieveAllEmployees();
